...
管线：Glass → shouldSkipNode → exact full-string → contextual → nodeValue。侧栏 Search 沿用 data-sidebar-menu-button；Appearance、General、Agents 的静态设置行分别要求 appearance、general、chat 页面标记及对应标签/说明容器；Settings 导航要求 glass-settings-sidebar 与 ui-sidebar-menu-button-label。初始扫描后，一个 childList MutationObserver 处理新增节点；thinking kind 与 ai role 也属于消息排除范围。
