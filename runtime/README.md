...
## Phase 2D.2（当前代码；General 七组文案已实机验收）
...
管线：Glass → shouldSkipNode → exact full-string → contextual → nodeValue。侧栏 Search 沿用 data-sidebar-menu-button；Appearance 的十三个标签与十三条说明要求 Glass、appearance 页面与对应标签/说明容器；General 的四组文案使用相同容器并要求 general 页面标记。初始扫描后，一个 childList MutationObserver 处理新增节点；thinking kind 与 ai role 也属于消息排除范围。
