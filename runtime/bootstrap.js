/**
 * cursor-agent-zh — Phase 2F.3 Agents Shortcut and Voice Labels
 *
 ...
 *
 * Phase 2F.3: keep 1C/1D.1/1D.2a scan + one Glass-only MutationObserver
...
(function cursorAgentZhBootstrap(root) {
  ...
  function isSettingsText(node, tab, classToken) {
    ...
    while (cur && guard < 64) {
      guard += 1;
      if (isElementNode(cur)) {
        if (!sawPanel && classNameOf(cur).split(/\s+/).indexOf(classToken) !== -1) {
          sawTarget = true;
        }
  ...
  function completeAsGlass(state, doc) {
    ...
    state.phase = '2f.3-agents-settings';
