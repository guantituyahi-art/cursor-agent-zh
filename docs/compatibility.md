...
> 锚定：**Cursor 3.21.18**。升级或其他版本安装前，应先完成本清单。  
> 相关脚本**尚未实现**；此处为人工/未来自动化检查项。  
> Checksum 策略与 `docs/architecture.md` §B 对齐。

## 1. 版本与安装完整性

- [ ] 确认 Cursor 版本为期望值（当前实验目标 3.21.18）
...
- [ ] 修改任何 Cursor 文件前已创建备份（未来：`scripts/` 备份工具）
- [ ] 备份目录不在将被覆盖的安装路径内（或另有副本）
- [ ] 已演练还原步骤
- [ ] 备份元数据包含 Cursor 版本与文件哈希
...
5. 若探针通过且目标可安全改动 → 备份 → 再应用 CSS /（未来）运行时加载注入  
