...
test('deploy loadTranslationsPack layered schema', () => {
  ...
  assert.strictEqual(pack.runtimePhase, '2B.2');
  ...
  assert.strictEqual(pack.contextual.length, 14);
...
test('buildSidecarSource injects pack before bootstrap', () => {
  ...
  assert.ok(built.source.includes('Phase 2B.2 Appearance Settings Label Expansion'));
...
test('runtimePhase constant', () => {
  assert.strictEqual(safety.RUNTIME_PHASE, '2B.2');
});
